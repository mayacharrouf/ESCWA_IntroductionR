...
The dataset has two separate sheets as previously mentioned. One on individual variables, and the other on household variables. The exercise will be to display the average age of individuals based on their governorate. The *merge* function helps in combining the two sheets in one output (here named Merged_Data) using Household ID. The first example will be a simple **bar plot**.

In order to create a graph, three steps are needed: 1. A pipeline that links the data that will be used for the plot with the ggplot() function. 2. X and y axes specified in the function called *aes()*. 3. The choice of the graph type, for example here *geom_bar()*.

Note that the "+" sign in the end of the line works as a pipeline to connect the command with the next instruction.

For a more advanced graph, the task is to create a bar plot that displays the average age of individuals based on their governorate and gender. The additional new parameter in *aes()* is the difference between the previous bar plot and the new one. Therefore, any characteristic should be added to the *aes()* command.
...
Graphs can always be adjusted to feed the purpose of the data. The above graph can be illustrated in a better way instead of having males and females piled up making the graph confusing and uninformative.
